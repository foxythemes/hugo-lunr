...
HugoLunr.prototype.readFile = function(filePath) {
  ...
  var sectionId = path.basename(uri);
  var pageTitle = "";

  if (meta.data.title !== undefined) {
    pageTitle = meta.data.title;
  }

  if (sectionId !== "index" && pageTitle) {
    pageTitle = matter.read(path.dirname(filePath) + "/index.md").data.title;
  }

  uri = path.basename(path.dirname(filePath));
  var page = uri;
  uri = `${uri}/#${sectionId === "index" ? uri : sectionId}`;
